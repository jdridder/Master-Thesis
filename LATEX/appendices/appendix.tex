...
\section{Mean and covariance of kinetic parameters.}

\begin{table}[!h]
    \centering
    \caption[Mean of the kinetic parameters.]{
        Mean of the activation energies and pre-exponential collision factors
        determined by a simulated paramter fitting experiment with a Berty reactor.
        }
        \begin{tabular}{ccl}
            \toprule
            \multicolumn{3}{c}{$\boldsymbol{\mu}$} \\ 
            \midrule
            ln $k_{0,1}$        & $15.6$ & $\mathrm{mol \; kg_{cat}^{-1} \; Pa^{-1.1} \; s^{-1}}$\\
            $E_\mathrm{A,1}$    & \SI{7.46e4}{} & \si{\joule\per\mole}\\
            ln $k_{0,2}$        & $16.2$ & $\mathrm{mol \; kg_{cat}^{-1} \; Pa^{-1} \; s^{-1}}$ \\
            $E_\mathrm{A,2}$    & \SI{8.96e4}{} & \si{\joule\per\mole}\\
            \bottomrule
        \end{tabular}
    \label{app:tab:mean-kinetic-parameters}
\end{table}
\begin{table}[!h]
    \centering
    \caption[Covariance matrix of the kinetic parameters.]{
        Covariance matrix of the activation energies and pre-exponential collision factors.
        For simplicity, the units are omitted. They appear as the respective multiplication of the row and column indices' units.
        }
        \begin{tabular}{ccccc}
            \toprule
            \multicolumn{5}{c}{$\boldsymbol{\Sigma}$} \\ 
            \multicolumn{1}{c}{} & 
            ...
            \midrule
            ln $k_{0,1}$ & $0.680$ & $3.39\times 10^{3}$ & $0.736$ & $3.67\times 10^{3}$ \\
            $E_\mathrm{A,1}$ & $3.39\times 10^{3}$ & $1.71\times 10^{7}$ & $3.67\times 10^{3}$ & $1.85\times 10^{7}$ \\
            ln $k_{0,2}$ & $0.736$ & $3.67\times 10^{3}$ & $0.797$ & $3.97\times 10^{3}$ \\
            $E_\mathrm{A,2}$ & $3.67\times 10^{3}$ & $1.85\times 10^{7}$ & $3.97\times 10^{3}$ & $2.00\times 10^{7}$ \\
            \bottomrule
        \end{tabular}
    \label{app:tab:covariance-kinetic-parameters}
\end{table}
