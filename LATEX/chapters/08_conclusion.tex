...
In the challenging field of robust optimization, it is crucial
to quantify uncertainty as well as retain objective performance.
...
optimization. In addition, the integration of high fidelity first principles models is prohibitive in real time control.
Thus, efficient surrogate models are another driver of this research.
\newline
\newline
\noindent
...
the quantiles of predefined uncertain measurements. Further, a physics-constrained (PC)
...
PCNNs with only \SI{25}{\%} of the hidden neurons
...
But for small or noisy data sets, PCNNs portray a real advantage over
...
measurement position. Neither the vanilla, naive nor the PC ensemble
...
In the mere forward pass, the PC
...
to the critical contraint and more gentle control inputs.
...
\newline
\noindent
The implementation of a PC activation function enables an improvement in the
...
during the backward passes (eq.~\ref{results:eq:pc-effect-on-backpropagation}) evoke the major impact.
This type of activation function allows to include linear equality constraints into machine learning
architectures. Thus, it is not only useful in chemical process modelling but also
a valuable tool with a wide range of applications.