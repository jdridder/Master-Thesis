...
\section{Surrogate Setup}
To be able, to simulate the system forward in time, following iterative formulation is needed.
\begin{definition}
    \label{sur:def: general surrogate setup}
    \[  
    \begin{gathered}
        \begin{pmatrix}
            \vx^\uparrow \\
        ...
        \end{pmatrix}_{k+1}
        = \begin{pmatrix}
            Q_{\tau_1}(\vx_{k+1} | \bm{\phi}^\uparrow_k) \\
            \mathbb{E}(\vx_{k+1} | \bm{\phi}_{\text{nom},k}) \\
            Q_{\tau_2}(\vx_{k+1} | \bm{\phi}^\downarrow_k)
        \end{pmatrix}
        \eqcolon f_\text{UQ}
        \begin{pmatrix}
            \bm{\phi}^\uparrow_k \\
