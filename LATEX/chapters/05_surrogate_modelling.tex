...
past measurements and inputs. Which must be known to initiate the RHS based a NARX framework.
\newline
...
To quantify the uncertainty for multiple states or measurements, 
...
the number of states or measurements with uncertainty quantification.
Here, a trade-off becomes clear – the framework allows to capture the effect of high-order uncertainty
vectors $\vw$ for a small amount of target states or measurements. In contrast, multi-stage MPC
...
\newline
\noindent
To realize physics-constrained NN outputs, several strategies exists. Physics-informed neural networks (\emph{PINN}s) for instance,
...
using the additional input $\bm{z}_0$ and a projection matrix $\bm{K}$. This projection matrix only contains the constraint matrix $\bm{A}$ can be computed
...
of physics-constrained NNs with parametric uncertainty for MPC, the focus remains on linear equality constraints.
...
\texttt{register\_buffer()} method. An implementation example of the physics-constrained activation can be found in the appendix (\ref{app:code:forward-pass}).
