...
Second, \emph{quantile}-NARX models are trained to simulate confidence intervals forward in time. Third,
...
\begin{definition} General RHS equation for uncertainty quantification.
    ...
    \begin{gathered}
        ...
        \end{pmatrix}_{k+1}
        = \begin{pmatrix}
            \hat{Q}_{\tau_1}(\vy_{k+1} | \bm{\phi}^\uparrow_k) \\
            \mathbb{E}(\vy_{k+1} | \bm{\phi}_{\text{nom},k}) \\
            \hat{Q}_{\tau_2}(\vy_{k+1} | \bm{\phi}^\downarrow_k)
        \end{pmatrix}
        \eqcolon f_\text{UQ}
        \begin{pmatrix}
            \bm{\phi}^\uparrow_k \\
...
quantiles for all measurements at the next time instant $k+1$ with the given feature vector for past measurements
and inputs $\bm{\phi}_k$. The measurements pertaining to the upper quantile function, are indicated
with "$\uparrow$". Similarly, the lower quantile measurements are denoted with a "$\downarrow$", while
"nom" refers to the nominal scenario. Technically, three RHS functions are propagated
independently trough time except for the initial state, which is the same for all three.
...
past measurements and inputs. Which must be known for initiation.
\newline

\section{NARX Framework for Chance Constraint Satisfaction}

When recalling the bi-level formulation of a semi-infinite program (def.~\ref{theory:def: bi-level program}),
worst case functions $\varphi_i(\vu)$ are assumed to ensure robust constraint satisfaction.
For simplicity, only measurement constraints in the 
form of $\vy_k \leq 0 \; \forall k$ are considered. This resembles a great amount of cases, as constraints
often appear as pure measurement constraints.  So, the uncertainty quantification in form of 
a quantile NARX model only needs to cover the measurements, that are part of measurement constraints.
Furthermore, quantile regression for measurements with counteracting dynamics lead to wrong predictions.
Therefore, it is reasonable to separate the prediction of quantile measurements from the prediction of all other measurements.


\begin{definition} Modified function $\hat{Q}_\tau$ to predict quantiles of critical measurements and according expected values
    of all other measurements.
    \[
        \hat{Q}_\tau(\vy_{k+1} | \bm{\phi}_k) = 
        \begin{pmatrix}
            Q_\tau(\vy_{k+1, :n_\text{crit}} | \bm{\phi}_k) \\
            \mathbb{E}_{\tau}(\vy_{k+1, n_\text{crit}:} | \bm{\phi}_k)
        \end{pmatrix}
    \]
\end{definition}
\noindent
Only constraint critical measurements $\vy_{:n_\text{crit}}$ undergo real quantile regression by the function $Q_\tau$.
The quantity $n_\text{crit}$ stands for the ending index of constraint critical measurements in the vector $\vy$.
All other measurements are to be inferred such that they match the quantile prediction of the critical measurements. 
This is marked by an expected value $\mathbb{E}_\tau$, that is conditioned by the quantile of the critical measurements.
...
To quantify the uncertainty for multiple measurements, 
three additional NARX models must be set up, one for each quantile and one for the nominal case.
In fact, the number of NARX models growths as a multiple of three which limits
the number of measurements with uncertainty quantification.
Here, a trade-off becomes clear – the framework allows to capture the effect of high-order uncertainty
vectors $\vw$ for a small amount of target measurements. In contrast, multi-stage MPC
is able to handle all uncertain states at once with a restriction on the number of uncertain parameters.
Additionally, it requires a state estimation routine.
...
values of those measurements, that are not inferred by quantile regression.
To train a NARX model $\mathbb{E}_\tau$, that learns
the expected values for the other measurements, that belong to the quantile predictions of the critical measurements,
...
\begin{equation}
    ...
    d = || Q_\tau(\bm{\phi}_k) - \vy_{k+1,:n_\text{crit}}||_2
\end{equation}
...
\begin{equation}
    ...
    \frac{1}{m} \sum^m_i \omega(d^{(i)}) \cdot
    || \vy^{(i)}_{k+1} - \mathcal{NN}(\bm{\theta}, \bm{\phi}^{(i)}_k) ||_2^2
\end{equation}
These distances
are used to calculate certain weights $\omega$ for a weighted MSE to serve as the loss function for 
$\mathbb{E}_\tau$ (eq.~\ref{sur:eq: weighted mean squared error}). To train such a conditioned NARX model
following steps must be executed (alg.~\ref{sur:algo: weighted MSE creation}).

\begin{algorithm}
	\caption{Calculation of the quantile based weights for a weighted MSE.}
	\label{sur:algo: weighted MSE creation}
	\begin{algorithmic}
		\REQUIRE $\bm{\Xi}$, $\mathbb{Y}$, $Q_\tau$
        \STATE $\mathbb{Y}_\text{crit} \leftarrow \mathbb{Y}_{:n_\text{crit}}$
        \STATE $\tilde{\mathbb{Y}}_\text{crit} \leftarrow Q_\tau(\bm{\Xi})$
        \STATE $\bm{d} \leftarrow || \mathbb{Y}_\text{crit} - \tilde{\mathbb{Y}}_\text{crit} ||_{(\cdot)}$
        \STATE $\bm{\omega} \leftarrow \omega(\vd)$
        \RETURN $\bm{\omega}$
	\end{algorithmic}
\end{algorithm}
...
vectors and $\mathbb{Y}$ the data matrix of measurements at the next time instant. The vector $\vd$ contains the
distances of the inferred quantile predictions to the ground truth in the critical measurements.
These are passed to a weight function $w$ to generate a weight vector $\bm{\omega}$, that collects all weights
for all data points.

\subsection{Weight Function}

To weight the data points in the MSE using the distance measure $d$, a lot of possible functions exist.
This is a straight tuning question. Inspired by the distributive character of uncertain measurements, that 
...
\begin{definition} Gaussian-tube as weight function.
    ...
    \[
    \omega(d) = \frac{2}{\sqrt{2 \pi} \sigma_\text{w}} \cdot e^{-\frac{1}{2} \frac{d^2}{\sigma_\text{w}^2}} 
...
for all non-quantile measurements, can be a source of error. The training of the conditioned model $\mathbb{E}_\tau$
...
By definition, neural networks (NNs) are designed to reproduce their training data in a best possible manner,
defined by the characteristics of the loss
function used during the training process. NN predictions minimize the loss towards the training data seen beforehand.
Circumstances exist, in which the output of an NN should strictly satisfy a certain set of equations. In case of
...
when predictions are propagated through several NNs or simulated in an autoregressive manner, errors accumulate. 
Final predictions are untrustworthy or simulations exhibit instability \cite{chen2024}. As an MPC algorithm needs a reasonable
...
Let the output of an NN be defined as well as a set of linear equality contraints that summarize some physical conservation quantity (def.~\ref{def: physics consistency}).
...
The variable $\bm{z}_0$ illustrates an external input into the constraint. This may be an NN input, a constant, or another batched 
...
$n_\text{out}$ and $n_\text{c}$ describe the number of output features and numbers
...
\begin{definition} Minimization of the euclidian distance towards the NN output as a linear equality constrained optimization.
    \label{def: physics constrained optimization}
    \[
        \begin{aligned}
            \bm{y}^* = \underset{\bm{y}}{\mathrm{argmin}} || \bm{y} - \bm{\tilde{y}} ||_2^2 
            \\
            \mathrm{s.t.} \quad \bm{A} (\bm{y} - \bm{z}_0) - \bm{b} = \bm{0}
        \end{aligned}
    \]
\end{definition}
\noindent
This means, the output of the network $\bm{\tilde{y}}$, the optimal solution $\bm{y}^*$ and $\bm{z}_0$ are vectors of
dimension $n_\text{out}$. 
The constraint matrix $\bm{A}$ must be of shape
$n_\text{c}~\times~n_\text{out}$. The vector $\bm{b}$ represents the offset of the linear equality constraints. This optimization
...
\begin{corollary} The optimal solution in the feasible space as projection of an activation function.
    ...
    = 
    \begin{pmatrix} 2 \bm{\tilde{y}} \\ \bm{A} \bm{z}_0 + \bm{b} \end{pmatrix}
    \\
    g_\text{pc}(\bm{\tilde{y}}, \bm{z}_0) \coloneq
    \bm{K} \begin{pmatrix} 2 \bm{\tilde{y}} \\ \bm{A} \bm{z}_0 + \bm{b} \end{pmatrix}
    = \begin{pmatrix} \bm{y}^* \\ \bm{\nu}^* \end{pmatrix}
    \\
    \mathrm{with} \quad \bm{K} = 
        \begin{pmatrix}
            2 \bm{I} & \bm{A}^\top  \\
            \bm{A} & \bm{0}   
        \end{pmatrix} ^{-1} 
...
using the additional input $\bm{z}_0$ and a projection matrix $\bm{K}$.
This projection matrix only contains the constraint matrix $\bm{A}$ and can be computed
...
\begin{figure}
    ...
    a feedforward NN, which is projected by the physics-constrained activation function $g_\text{pc}$
...
In most use cases however, it is applied as a final correction function at the output (fig.~\ref{theory:fig: pc_neural_network}).
...
To clarify the terminology, the naming physics-\emph{constrained} is used, because an actual hard constraint is applied.
Physics-consistent NNs however, may implement arbitrary physical knowledge, whereas PINNs include physical equations into the loss function.
Thus, physics-constrained (PC) NNs and PINNs are both physics-consistent NNs.
Furthermore, the procedure could be extended
...
of PCNNs with parametric uncertainty for MPC, the focus remains on linear equality constraints.
...
\noindent
The forward pass through the PC activation involves the setup of a temporary vector $\vh$ (alg.~\ref{theory:algo: pc forward pass})
...
\texttt{register\_buffer()} method. An implementation example of the PC activation can be found in the appendix (\ref{app:code:forward-pass}).
For a straightforward implementation, the formulation appears in such a way that it supports batching of 
a certain batch size $n_\mathcal{B}$.
Computing frameworks that support broadcasting such as \texttt{numpy} or
\texttt{pyTorch} allow the implementation of $\bm{z}_0$ in the shape of $1 \times n_\text{out}$ to reduce memory usage as long as it remains constant.