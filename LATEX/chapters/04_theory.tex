...
In a probabilistic environment, the prediction of confidence intervals in the form of quantiles may be of major interest.
...
the respective upper and lower quantiles are defined (eq.~\ref{theory:eq: quantiles from confidence}).
\begin{equation}
    \label{theory:eq: quantiles from confidence}
    \tau_1 = \alpha/2 \quad \text{and} \quad \tau_2 = (1 - \alpha/2)
\end{equation}
To correct the interval prediction of the corresponding quantile regressors $\tilde{q}_{\tau_1}$ and $\tilde{q}_{\tau_2}$.
\begin{equation}
...
Furthermore, \emph{quantile regression} (\ref{theory: quantile regression}) is used to quantify the uncertainty of system states. 
...
quantile regression can be used to directly estimate given quantiles $\tau$.
...
of account for uncertainties during MPC runtime. This outsources computational effort out of the dynamic optimization loop.
\newline
\newline
Upper and lower
...
Quantile models $Q_\tau$ are trained using the \emph{quantile loss} function (eq.~\ref{theory:eq: quantile loss}).
Conformalization can be carried out, to calibrate quantile predictions
which marginally guarantees a certain coverage of the distribution by adjusting the intervall widths \cite{romano2019cqr,kemmerling2025}. 
As the focus lies on the mere effect of physics-consistency, conformalized quantile regression (CQR) is 
not put to use for better interpretability.
