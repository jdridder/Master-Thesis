...
Before diving into the application of a special model predictive control (MPC) algorithm. Uncertain
...
well as retain the \emph{performance} with respect to (w.r.t.) the objective function.
\newline

\section{Robust Optimization}

The following optimization task (\ref{theory:def:semi-infinite-program}) under an uncertainty vector $\vw$
may be considered.
\begin{definition} Semi-infinite program with the uncertainty $\vw$.
    \label{theory:def:semi-infinite-program}
    \[
    \begin{gathered}
...
To reduce the number of constraints, the worst case estimate can be applied here (\ref{theory:def: bi-level program}). 

\begin{definition} Bi-level formulation of the semi-infinite program
    \label{theory:def: bi-level program}
    \[
    \begin{gathered}
        \underset{\vu}{\min} \; \underset{\vw}{\max} \quad F_0(\vu, \vw) \\
...
exactly describes the worst case for $F_i$ parametrized by $\vu$. In fact, if certain assumptions hold
...
With the universal approximation capability of neural networks (NNs) in mind \cite{hornik1990}, 
...
problem needs to be solved recursively at given time instants.
Chemical plants usually exhibit \emph{slow dynamics}, meaning, they react slowly to their inputs.
Other physical systems with fast dynamics, like quadrocopters behave much faster. So, the
...
\begin{definition} Continuous-time formulation of an MPC problem.
    ...
    \[
        \begin{gathered}
            ...
            \begin{aligned}
                \text{s.t.} \quad &\dot{\vx} = f(\vx, \vu) \\
                & F_i(\vx, \vu) \leq 0 \quad \forall t \in [t_0, t_\text{end}] \\
                & \vx_0 = \vx_\text{init}
            \end{aligned}
        \end{gathered}
    \]
...
the dynamics of the system, which are described by the system model $f(\vx, \vu)$. Thus, the quality
...
instability and thus damage of the real system.
For instance, a \emph{temperature runaway} must be avoided at all costs in a chemical reactor.
This happens, if the exothermic heat release exceeds the maximum possible
...
are an important field of research in the discipline of optimal control.
...
This MPC formulation (def.~\ref{theory:def: continuous mpc}) is infinite in time and in the uncertain parameters.
...
\begin{figure}[!h]
    ...
    horizon $N_\text{p}$ \cite{lucia2013}.
    }
    \label{theory:fig: multi-stage-mpc}
\end{figure}

\noindent
It generates a scenario tree (fig.~\ref{theory:fig: multi-stage-mpc}) for all pre-defined combinations of disturbances.
In the algorithm design phase, scenario cases $w_k^{(\cdot)}$ must be
...
branches for every discrete uncertainty case are generated. This procedure is coined \emph{scenario generation}.
In practice, the worst cases for the disturbances are assumed plus an optional expected value. Mostly, this yields good results
...
the number of parameters and their respective values (eq.~\ref{theory:eq: number scenarios}). 
...
\emph{Feedforward neural networks} (NNs), sometimes referred to as \emph{multi-layer-perceptrons} (MLPs),
...
layers and respective neurons is high enough, up to an arbitrary small error \cite{hornik1990}. Thus, NNs play a major role in
recent developments in machine learning. Large language models such as ChatGPT heavily rely on neural
networks \cite{vaswani2023}. NNs are not only succesfully applied in natural language processing but also in the chemical process industry.
...
Their ability to approximate any non-linear function while being differentiable, makes NNs especially suitable for dynamic optimization
such as MPC. In contrast to detailed first principle models, NNs have the advantage of making a prediction with
less non-linear function evaluations and floating point operations. Thus, the computational complexity may be lowered significantly.
 As MPC operates in real time parallel to the system to be controlled, the computation duration is a key measure
for a successful MPC implementation. That is why, surrogate models in the form of NNs are a crucial tool for dynamic
optimization and are heavily investigated in current research \cite{salzmann2024l4casadi}. May an NN be given as an alternating series of function evaluations
between a linear function $h(\bm{a})$ and a \emph{non-linear} activation function $g(h(\va))$ (def.~\ref{theory:def: neural network}).
...
\noindent
The linear transformation matrices $\bm{\theta}_{Wl}$ and offsets $\bm{\theta}_{bl}$
are called weights and biases respectively and are the trainable parameters of an NN. The activation functions $g$ are important
...
\begin{equation}
    ...
    \begin{aligned}
        ...
        \text{with} \quad \bm{\tilde{y}}(\bm{\theta}, \bm{\xi}^{(i)}) &\coloneq \mathcal{NN}(\bm{\theta}, \bm{\xi}^{(i)})
    \end{aligned}
\end{equation}

\noindent
In \emph{supervised learning}, the training data is separated in \emph{features} $\mathbb{X}_\text{train}$, the inputs of the NN,
and \emph{targets} $\mathbb{Y}_\text{train}$. During training, the parameters $\bm{\theta}$ are determined such that
the NN approximates the data in best possible manner.
In this example, the loss function $\mathcal{L}$ portrays the mean squared error (MSE), that is used in most regression tasks. 
The difference between the model prediction, denoted as $\bm{\tilde{y}}(\bm{\xi}^{(i)})$, and the true data point $\bm{y}^{(i)}$
is penalized quadratically to learn the expected value of a data distribution.

\subsection{Quantile Regression}
\label{theory: quantile regression}

In probability theory, the quantiles are a foundational measure of distribution width. Given a certain 
order $\tau$, its quantile definition is recalled as follows \cite{lange2017}.
\begin{definition} Quantile function of order $\tau$.
    \label{theory:def:quantile}
    \[
    P(q_\tau) = \int_{-\infty}^{q_\tau} p(\rx = x) dx = \tau
    \]
\end{definition}
\noindent
Where $p(\rx = x)$ represents the probability density function of the random variable $\rx$. The
$\tau$-th quantile is reached, when the probability mass $P(q_\tau)$ is equal to the quantile order.
To specify the prediction characteristic of an NN, the type of training loss function is decisive.
By utilizing a loss function in form of the MSE, an NN is trained to approximate the expected value of a distribution of data points.
...
\end{equation}
\noindent
The hyperparameter $\tau$ denotes the order and therefore position of the quantile. A small value for $\tau$, e.g.,
...
the threshold $y \geq \tilde{y}(\bm{\theta})$ to include a greater amount of points.
...
constraints $F_i(\vu, \vw)$ (eq.~\ref{theory:def: bi-level program}) in terms of confidence quantiles. This would reduce
the semi-infinite program to an uncertainty-finite program.

\subsection{Conformalized Quantile Regression}
\label{theory: CQR}

Quantile regressors trained by this approach achieve their specified coverage approximately. 
Nonetheless, for data with strong heteroscedasticity the predicted confidence interval may be too
tight or too narrow at local variations \cite{romano2019cqr}. \emph{Conformalized Quantile Regression}
(CQR) is capable of calibrating the interval predictions of arbitrary quantile models to achieve
a theoretical guarantee of the desired coverage. This is done, by calculating a conformity score with a
trained quantile model and an unseen \emph{calibration} data set. Given a certain confidence level $\alpha$
with the corresponding coverage $(1-\alpha)$,
the respective upper and lower quantiles are defined (def.~\ref{theory:def: trained quantile regressors}).
\begin{definition} Quantile regressors trained by an arbitrary algorithm.
    \label{theory:def: trained quantile regressors}
    \[
    \begin{gathered}
        \begin{pmatrix}
            \tilde{q}_{\tau_1}(\bm{\xi}), \tilde{q}_{\tau_2}(\bm{\xi})
        \end{pmatrix} \leftarrow
        \mathcal{A} \big (\mathbb{X}_\text{train}, \mathbb{Y}_\text{train} \big) \\
        \text{with} \quad \tau_1 = \alpha/2 \quad \text{and} \quad \tau_2 = (1 - \alpha/2)
    \end{gathered}
    \]
\end{definition}
\noindent
With any existent algorithm $\mathcal{A}$ and the corresponding training data $\big (\mathbb{X}_\text{train}, \mathbb{Y}_\text{train} \big)$,
two quantile regression models $\tilde{q}_{\tau_1}$ and $\tilde{q}_{\tau_2}$ can be fitted (def.~\ref{theory:def: trained quantile regressors}).

\begin{equation}
    \label{theory:eq: conformity scores}
    \begin{gathered}
        \bm{\xi}^{(i)} \in \mathbb{X}_\text{cal} \quad 
        \vy^{(i)} \in \mathbb{Y}_\text{cal} \quad
        \text{with} \quad i \in \{0, 1, ..., m_\text{cal} - 1\}
        \\ 
        \bm{\epsilon}^{(i)} \coloneq \max(\tilde{q}_{\tau_1}(\bm{\xi}^{(i)}) - \vy^{(i)}, \vy^{(i)} - \tilde{q}_{\tau_2}(\bm{\xi}^{(i)}))
    \end{gathered}
\end{equation}
\noindent
Using a second calibration data set $\big (\mathbb{X}_\text{cal}, \mathbb{Y}_\text{cal} \big)$, a conformity score
$\bm{\epsilon}$ is computed, that acts as an empirical measure of how well the given
quantile predictions cover the data (eq.~\ref{theory:eq: conformity scores}). If a data point $\vy^{(i)}$
falls into the interval, the score is the maximum of the negative distances to the estimated quantiles.
In case a calibration point is located below $\tilde{q}_{\tau_1}$, this distance is counted as a violation
in the positive direction. The same occurs, when a point lies above $\tilde{q}_{\tau_2}$. In summary,
coverage is counted as a negative and miscoverage as a positive in a quantitative manner.
The violation of the desired coverage of $(1-\alpha)$ is encoded into the conformity scores. 
Therefore, its respective empirical quantile acts as a correction term for the original quantile 
predictions (eq.~\ref{theory:eq: empirical quantile of scores}).

\begin{equation}
    \label{theory:eq: empirical quantile of scores}
    Q_{1-\alpha}(\bm{\epsilon}) \coloneq (1-\alpha)(1+1/m_\text{cal})\text{-th empirical quantile of} \; \bm{\epsilon}
\end{equation}
\noindent
\noindent
It must be substracted
from the lower quantile prediction and added to the upper quantile prediction
to yield the calibrated regressors  $q_{\tau_1}$ and $q_{\tau_2}$ (th.~\ref{theory:th:CQR-update}).

\begin{theorem} Adjusted prediction interval with the empirical quantile of the conformity scores
     $\bm{\epsilon}^{(i)}$ \cite{romano2019cqr}.
    \label{theory:th:CQR-update}
    \[
        \begin{pmatrix}
            q_{\tau_1}(\bm{\xi}), q_{\tau_2}(\bm{\xi})
        \end{pmatrix} = 
        \begin{pmatrix}
            \tilde{q}_{\tau_1}(\bm{\xi}) - Q_{1-\alpha}(\bm{\epsilon}), \tilde{q}_{\tau_2}(\bm{\xi}) + Q_{1-\alpha}(\bm{\epsilon})
        \end{pmatrix}
    \]
\end{theorem}
\noindent
Once calculated, the calibration term can be stored as an untrainable model parameter.
In essence, CQR allows to improve the interval prediction of any quantile regression model in a 
straightforward fashion. Its simplicity and general applicability makes it a powerful tool 
for any probabilistic prediction framework. For further background,
it is referred to the original work by Romano et al. \cite{romano2019cqr}.
...
When an NN is trained with data generated by a first principle model, the simulation is done offline from the MPC algorithm.
This can significantly reduce the online solution time in contrast to first principle models \cite{salzmann2023}, because
the latter require an expensive integration routine. The general idea here is to forecast the measurements at a future
time instant or instants as a function of past measurements and inputs (def.~\ref{theory:def: NARX Model}).

\begin{definition} Time-discrete form of an NN to predict the future state based on past information.
    \label{theory:def: NARX Model}
    \[
     \begin{aligned}
        \vx_{k+1} &= f(\vx_k, \vu_k) \\
        \vy_{k} &= h(\vx_k) \\   
        \bm{\tilde{y}}_{k+1} &= \mathcal{NN}(\vy_k, \vy_{k-1}, ..., \vy_{k-h}, \vu_k, \vu_{k-1}, ..., \vu_{k-h})
     \end{aligned}
    \]
\end{definition}
...
state $\vx_k$ through a function $f(\vx_k, \vu_k)$ that discribes the system dynamic (def.~\ref{theory:def: NARX Model}).
...
with the abbreviation \emph{right-hand-side} (rhs). Since the states are often unaccessable for measuring in a real system,
a measurement function $h$ maps the states to a measurement vector $\vy$, which can be effectively captured by process analytics.
Because of that, measurement noice is introduced into the vector $\vy$. Another advantage of NNs is, that they operate entirely 
without states. That is why, a state estimation technique becomes obsolete.
\newline
\newline
The measurement propagation by the $\mathcal{NN}$ is a classical example of time series extrapolation. NNs have shown significant
capabilities in time series forecasting. Deep learning based architectures exist for time series forecasting
in various complexities. Convolutional NNs (CNNs), Long short-term memory (LSTMs), and Transformers are the most prominent for
time series prediction with varying complexity \cite{benidis2022,vaswani2023}.
These provide a great opportunity to apply deep learning based forecasting within optimal control algorithms, where the next
measurement $\bm{\tilde{y}}_{k+1}$ is the output of a neural network.
In context of this work, shallow feedforward NNs or \emph{single-layer perceptrons} (SLPs) are used for this purpose, which
technically belong to \emph{shallow learning} architectures.
\newline
\newline
...
the future prediction is a function of a series of past measurements \{$\vy_k, \vy_{k-1}, ..., \vy_{k-h}$\}, while \emph{exogenous}
implies that a series of outside control inputs \{$\vu_k, \vu_{k-1}, ..., \vu_{k-h}$\} is passed to the function as well (def.~\ref{theory:def: NARX Model}).
...
In fact, the determination of which and how many instants of past inputs and measurements to introduce, is part
of hyperparameter tuning. As this would inflate the work unnecessarily, $h$ is set equal for states and inputs.
Furthermore, the measurements are supposed to equal the states. Implicitly, the states are directly measurable.
...
For large systems, the input length increases the network size unacceptably quick, while the input time series often exhibits
...
a lower-dimensional subspace via a linear orthogonal transformation. This subspace represents the space of the input data with the biggest variance.
This procedure is built on the concept of \emph{singular value decomposition} (SVD).

\subsection{Singular Value Decomposition}
\label{theory: svd}
SVD is a fundamental building block in linear algebra. Its applications
range from image compression, over exploratory data analysis to model reduction in fluid mechanics.
...
via the respective \emph{singular vectors}. The compression and component identification is guaranteed
...
This is especially true for data points in close temporal or spatial distance. Because the SVD is the basis for
...
Its computation can be implemented via an incremental PCA, that supports GPU acceleration and memory efficient batching
with \texttt{pyTorch} \cite{pytorch2019, incrementalpca2025}.
...
\begin{equation}
    ...
    \begin{gathered}
        ...
        \text{with} \quad \bm{\Psi} \coloneq \bm{U}_{:,:r} \in \mathbb{C}^{n \times r}
        \quad \bm{\tilde{\Sigma}} = \bm{\Sigma}_{:r,:r} \in \mathbb{R}^{r \times r}
        \quad \bm{\tilde{V}}^* = \bm{V}^*_{:r,:} \in \mathbb{C}^{r \times m}
    \end{gathered}
\end{equation}
...
\begin{equation}
    \label{theory:eq: svd projection}
        \begin{gathered}
            ...
            \text{with} \quad \bm{\tilde{X}} \in \mathbb{C}^{r \times m}
        \end{gathered}
\end{equation}
...
It can be proven, that this approximation is the best existing one with respect to the Frobenius norm as it solves following minimization \cite{Brunton2019} (eq.~\ref{theory:eq: svd minimization}).
\begin{equation}
...
can be compressed via SVD into a low-dimensional vector $\bm{\xi}_k$ that, when multiplied with the basis vectors,
approximates the original input (fig.~\ref{theory:fig: narx-pca-encoder}). This is done 
by a \emph{PCA encoder} placed before the first layer of a NARX model to reduce the network size. 
...
\section{Quantification of Model Uncertainty}

Furthermore, CQR (sec.~\ref{theory: CQR}) is used to quantify the uncertainty of system states. 
Under model uncertainty, the evolvement of the system described by $f(\vx, \vu, \vw)$ heavily
depends of the uncertainty vector $\bm{w} \in \mathbb{W}$ (def.~\ref{theory:def: continuous mpc}).
While multi-stage MPC roughly discretizes the parameter space for parameter extreme cases under tremendous computational effort,
CQR can be used to directly estimate given quantiles $\tau$.
This has the advantage, that the uncertainty quantification is done independently from the disturbances. As non-linear
systems behave non-monotonous to uncertainties, extreme parameter cases do not necessarily account for
system extreme cases. Explicit quantiles bound the system states with a given coverage lumping all
possible disturbance effects together.
Another advantage is, that the uncertainty quantification is done offline by training a quantile model beforehand instead
of accounting for uncertainties during MPC runtime. This outsources computational effort out of the dynamic optimization loop.
\newline
\newline
Upper and lower
NARX quantile models can be trained, to predict a confidence interval that includes predictions of future states
with a given coverage. In total, three predictions must be made to simulate the system forward in time
while quantifying the uncertainty. These consist of the expected value of the measurements at the next instant,
which is denoted as $\mathbb{E}(\vy_{k+1}|\bm{\phi}_k)$, and two quantiles, $Q_\tau(\vy_{k+1}|\bm{\phi}_k)$,
with pre-defined probability coverages $\tau$. All are predicted given the feature vector $\bm{\phi}_k$
of past measurements and inputs. In this work, models that predict the expected value $\mathbb{E}$ are trained 
using the \emph{mean squared error} as loss function (eq.~\ref{eq: loss function}). 
Quantile models $Q_\tau$ are trained using the \emph{quantile loss} (eq.~\ref{theory:eq: quantile loss})
in combination with CQR (sec.~\ref{theory: CQR}).
...
species in a reaction network. The sum of all atoms in all species must be equal at all times regardless of how the underlying reactions take place.
...
In reaction system analysis, the element-species-matrix is directly tied to the matrix
of stoichiometric coefficients denoted as $\bm{N}$. In fact, it acts as the null space of $\bm{N}$
(eq.~\ref{theory:eq: null space stoichiometric matrix}) \cite{guttel2021} .
\begin{equation}
    \label{theory:eq: null space stoichiometric matrix}
    \bm{B}\bm{N} = \bm{0}
\end{equation}
Where $\bm{0}$ stands for the zero matrix. So utilizing the null space of the matrix of stoichiometric
coefficients or the element-species-matrix $\bm{B}$ is actually equivalent.
