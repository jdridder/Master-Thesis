...
Their ability to approximate any non-linear function while beeing differentiable, makes NNs especcially suitable for dynamic optimization
...
in current research \cite{salzmann2024l4casadi}. May a NN be given as an alternating series of function evaluations
...
and \emph{labels} $\mathbb{Y}_\text{train}$. During training, the parameters $\bm{\theta}$ are determined such that
...
By utilizig a loss function in form of the MSE, a NN is trained to approximate the expected value of a distribution of data points.
To specify the prediction characteristic of a NN, the type of training loss function is decisive.
...
When a NN is trained with data generated by a first principle model, the simulation is done offline from the MPC algorithm.
...
For large systems, the input length increases the network size unacceptably quick, while the input time serieses often exhibit
