...
Especcially with regard to \emph{load flexible} processes using renewable energy sources, dynamic control such as MPC can enable optimal operation. For example, the energy supply can be closely tied to weather conditions or the feed composition may fluctuate due to
...
component $\alpha$ and $K_j$ the adsorption coefficient for carbon dioxide. The collision factor and the adsorption
...
from an optimal design task (fig.~\ref{case:fig: complete reactor}). To ensure thermal stability, the additional temperatureprofile derivation is mandatory. 
...
The technical reactor itself is equipped with four wall cooling sections ($T_{\text{w,1}}$ to $T_{\text{w,4}}$) to manipulate the temperature profile.
The input flow velocity $u$ can fluctuate due to flexible feed availability such as in carbon dioxide methanization,
where the throughput heavily relies on the hydrogen supply \cite{}. Because isobaric conditions and a non-compressible phase are assumed,
...
It not only improves numerical simulation stability, for later training of surrogate models, it also eliminates the need for
...
\newline
\newline
The enthalpy balance in local form can be significantly reduced under isobaric conditions with neither volume forces,
...
exactly determined. They lie in a confidence space. Furthermore, fitting results with counteracting mechanisms
...
can be determined for a cross-correlated parameter fitting (def.~\ref{case:def: multi-variate distribution}).
...
measurement noise has been executed using a \emph{berty} reactor model. All probabilistic terms are summarized (tab.~\ref{}).
...
\begin{figure}
    ...
    factor show severe non-linear uncertainty. The nominal value is marked in yellow.}
    \label{case:fig: kinetic parameter distribution}
\end{figure}

\begin{tabular}{lr | rrrr}
\multicolumn{2}{c}{$\bm{\mu}$} & \multicolumn{5}{c}{$\bm{\Sigma}$} \\ 
\cmidrule(l){3-6}
\multicolumn{1}{c} $x$ & 1 & $\ln k_{0,1}$ & $E_{\text{A}, 1}$ & $\ln k_{0,2}$ & $E_{\text{A}, 2}$ \\
\midrule
