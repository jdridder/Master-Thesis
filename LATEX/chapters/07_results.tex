...
These include the computational efficiency of the NARX models during the offline training phase,
the degree of balance violation and the adequacy of the uncertainty quantification.
Furthermore, it is important, how the physics contrained activation function alters MPC runtime.


\section{Training Effectivity}

The \emph{validation losses} during the training of several different pc-NARX (solid line) and a vanilla-NARX models (dashed line) are compared
...
of $4$ hidden neurons represents a challenge. The training is performed with identical learning rate
of \SI{5e-5} and a batch size of $1024$ for all three NARX variations using \SI{1.024e6}{} data points.
...
the same validation loss at first sight.
\newline
\newline
It is questionable however, whether the pc-NARX can maintain its advantage for a growing number of training epochs.
As the balance constraint implemented into the pc-NARX is present in the data, the vanilla-NARX
will learn it accordingly, if the number of epochs is sufficiently high. The minimum possible 
validation loss is determined by the network size, the amount and \emph{quality} of available training data and the training duration.
So theoretically, the vanilla-NARX should approach the pc-NARX in exessive neural network training
when a sufficiently large amount of high-quality data is supplied.
\newline
\newline
In reality, training data is often noisy and available in a limited amount.
...
physics-constrained NNs perform much better than basic NNs when the amount training
