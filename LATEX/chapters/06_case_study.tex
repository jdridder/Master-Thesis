...
Especially with regard to \emph{load-flexible} processes using renewable energy sources, dynamic control such as MPC can enable optimal operation \cite{fischer2021}.
For example, the energy supply can be closely tied to weather conditions or the feed composition may fluctuate due to
biological resources \cite{dittmer2022}.
\newline
\newline
For exothermal reactions, \emph{transient behavior} is a major safety concern of packed-bed tubular reactors \cite{guttel2021}.
...
is considered a safety-critical state that needs to obey to a maximum constraint in the sense of $F_i \leq 0$ in the original optimization (def.~\ref{theory:def: bi-level program}) for all possible disturbances $\vw$.
...
conversion is set to be required via a constraint. The actual control formulation is presented during the
MPC performance evaluation (sec.~\ref{MPC-Performance-and-Robustness}). 

\section{Reaction Kinetics}
To focus on the surrogate modelling methology, the example system is kept rather simplistic. 
The \emph{ethylene oxidation} process consists of two parallel reactions. Parametric data
is readily available due to extensive reseach. Further, the reaction and 
reactor configuration is taken from an optimal reactor design study by Pietschak et al. \cite{pietschak2018} and
adjusted. Thus, some proposed operating conditions may be unrealistic due to safety concerns
...
The reactor as a packed-bed tubular reactor (TR) is assumed to be already existent with a fixed design to clearly separate the model derivation
from an optimal design task (fig.~\ref{case:fig: complete reactor}).
For the first principle model, the mass and energy balances are set up.
...
\begin{figure}
    ...
    one pseudo-homogenous reaction phase. The inlet conditions remain constant for the temperature $T$ and the 
...
linear equality constraints in an NN. 
Using the component balance in local form (eq.~\ref{case:eq: component balance in local form}), a PDE can be derived that
...
\begin{equation}
    ...
    \frac{\partial c_\alpha}{\partial t} = - \frac{\partial}{\partial z_k} (c_\alpha u_k + j_{\alpha, k}) + R_\alpha
\end{equation}
The general component density $c_\alpha$ is expressed in mol/m$^3$, while $\partial z_k$ indicates
the spatial derivative in direction $k$. $u_k$ and $j_{\alpha, k}$ denote the convective velocity and the 
diffusive mass transport in direction $k$. The rate of formation or depletion $R_\alpha$ allows to consider chemical reactions.
Ideal gas behavior is considered (eq.~\ref{case:eq: ideal gas law}).
\begin{equation}
    \label{case:eq: ideal gas law}
    p_\alpha = c_\alpha \mathrm{R} T
\end{equation}
...
A detailed derivation of the model equations can be found in the appendix (app.~\ref{app:eq:component-mass-balance}). 
Due to the ideal gas behavior (eq.~\ref{case:eq: ideal gas law}), the concentration $c_\alpha$ varies not only
with the mole fraction but also with the temperature. Therefore, an artificial state $\chi_\alpha$ is introduced
...
form indicated by a prime (') symbol. This is done by diving by a reference value, denoted with a star
(*) (eq.~\ref{case:eq: dimensionless component PDE}).
...
Reynolds and Prandl expressions are part of the appendix to keep the derivation tidy (app.~\ref{app:eq:reynolds-number}).
\begin{equation}
...
This discretization method is chosen against the flow direction, because it is convection dominated.
...
\begin{center}
    \begin{tabular}{c c c}
        ...
        \midrule
        $k_{0,j}$ & $6.275\times 10^6  \; \mathrm{mol \; kg_{cat}^{-1} \; Pa^{-1.1} \; s^{-1}}$ & $1.206\times10^7 \; \mathrm{mol \; kg_{cat}^{-1} \; Pa^{-1} \; s^{-1}}$ \\
...
measurement noise is executed using a \emph{Berty} reactor model.
To do so, the experimental setup by Al-Saleh et al. \cite{alsaleh1988} is modeled via a Berty reactor with ideal mixing
using the true kinetic parameters for the reaction kinetics.
Artificial gas chromatograph and temperature measurements are taken and pertubed by additive Gaussian noise.
Using $128$ measurement points for varying conditions, the parameter fitting is executed $1000$ times
to yield a data set of $1000$ combinations of the four kinetic parameters.
Finally, the mean and covariance of this data set are calculated and serve as the basis for the
Monte-Carlo (MC) sampling of the underlying multi-variate Gaussian distribution. 
Mean and covariance can be consulted in the appendix 
(tab.~\ref{app:tab:mean-kinetic-parameters} and \ref{app:tab:covariance-kinetic-parameters}).
...
Under the assumptions taken earlier, mainly ideal gas behavior and incompressibility, the change in molar amount
...
\label{case:measurement-positions}
...
the states at the element indices $32, 64, 96$ and $128$ are considered as the measurements.
Regarding the contraint satisfaction of the temperature, it is suitable to solely focus on the maximum temperature in the reactor.
But for NARX models to learn the system correctly, spatial information about the temperature profile is needed.
The outlet states are of major interest in all cases.
...
It consists of a nominal prediction $\mathbb{E}$ and two quantile predictions $\hat{Q}_\tau$ (fig.~\ref{case:fig: narx framework}). 
\begin{figure}
    \centering
    ...
    {
        To simulate the distribution of system trajectories (blue bell curve), three NARX predictors are applied in form
        of the expected value (green), the $0.9$-quantile (yellow) and the $0.1$-quantile (purple). Each of them consists
        of two separate NARX models, one for the temperature measurements $\bm{T}$ and one for all other states $\bm{\chi}$.
    }
    \label{case:fig: narx framework}
\end{figure}
All three predictors are separated in two parts.
As the temperature is the critical state regarding the constraints, it is predicted separately 
from all other states. This allows for more accurate temperature predictions, as well as explicit quantile regression
for the temperature measurements only. So the NARX models $Q_{0.9}$ (yellow) and $Q_{0.1}$ (purple) undergo 
quantile regression (\ref{theory: quantile regression}) in the training phase with the $0.9$- and $0.1$-quantile respectively.
The expected value of the temperature $\mathbb{E}(\bm{T}_{k+1})$ (green) is learned using the MSE.
The same is done for the expected values of all other states, namely the dimensionless concentration measures $\bm{\chi}$.
To predict all other states, that \emph{most likely} correspond to the temperature quantiles, NARX models are trained using 
a weighted MSE. These are marked as $\mathbb{E}_{0.9}$ and $\mathbb{E}_{0.1}$ to match their corresponding temperature quantile.
Training efficiency and predicion quality rely on the choice of distance measure and weight function (alg.~\ref{sur:algo: weighted MSE creation}),.
\newline
\newline
For one complete forward pass, the vector of past measurements and inputs $\bm{\phi}_k$ is normalized with a standard
scaler to have a mean of zero and a unit standard deviation. It is then encoded by the PCA encoder to a lower dimensional 
subspace (\ref{theory: svd}). The compressed vector $\bm{\xi}_k$ needs to be scaled again with a maximum scaler before
it can be fed into a NARX model. After the NARX models for the temperature and all other states have predicted the 
next instance, the complete measurement vector is constructed. For the next recursive forward pass, the complete
measurement vector is inserted as the first element into the vector of past measurements and inputs (eq.~\ref{theory:eq: input feature vector}).
Additionally, the applied input $\vu$ is inserted as well at its corresponding position. This recursive insertion process
is done by the function $f_\text{shift}$ using the previous vector of past measurements and inputs $\bm{\phi}_{k-1}$ (fig.~\ref{case:fig: narx framework}).
To start the simulation, an initial vector $\bm{\phi}_\text{init}$ must be supplied.
...
To train and test the NARX frameworks, reactor measurement data is generated at mentioned positions. 
For this sake, the normal distributions for the radial heat conductivity and the kinetic parameters 
are sampled in MC fashion to yield a parameter vector $\vw$ in the uncertainty space.
...
a specified sampling time constant. The ramp duration is set to half of the sampling time constant.
...
The heat transfer to the reactor wall is determined as the greatest time constant with
approximately \SI{20}{\second} (eq.~\ref{case:eq: time constant}).
\begin{equation}
    \label{case:eq: time constant}
    \tau_\text{h} = \frac{\rho c_\text{p} d_\text{t}}{4 \alpha} \approx \SI{20}{\second}
\end{equation}
\newline
\noindent
It is important to note that the maximum hydrodynamic residence time for the flow speed is greater than the time
scale of the wall heat transfer (eq.~\ref{case:eq:residence-time}).
\begin{equation}
    \label{case:eq:residence-time}
    \tau_u = \frac{L}{u} = \frac{\SI{10}{\meter}}{\SI{0.18}{\meter \; \second^{-1}}}
    = \SI{55.56}{\second}
\end{equation}
But the change induced by the flow speed is instantly applied to the system due to 
mass continuity. In this case of incompressibility, this translates even to continuity
in volume. Thus, the wall heat transfer acts as the slowest process
that, governs the dynamic behavior of the system. 
\newline
...
a horizon of \SI{512}{\second} with a sampling frequency of \SI{1}{\second^{-1}}. The wall temperature randomly
switches its levels between \SI{553}{} and \SI{665}{\kelvin} and remains for a random factor of the specified 
signal time constant. The exact configuration of the data generation is given (tab.~\ref{case:tab:data-generation-parameters}).
For the train and calibration data sets, $1000$ independent system trajectories are simulated with 
$512$ time steps of \SI{1}{\second} which yields \SI{1.024e6}{} data points each. The test set is
smaller with $100$ trajectories and a shorter horizon of \SI{512}{\second}.
\begin{table}
    \caption[Configuration for the data generation.]{
        Configuration for training, calibration and test data generation.
    }
    \centering
    \begin{tabular}{cccccc}
        \toprule
        & & & \multicolumn{3}{c}{\textbf{wall temperature}} \\
        purpose & trajectories & steps & bounds / \SI{}{\kelvin} & $\tau$ / \SI{}{\second} & vary \\
        \midrule
        train & 1000 & 1024 & (535, 665) & 12 & True  \\
        \cline{1-6}
        calibration & 1000 & 1024 & (535, 665) & 12 & True  \\
        \cline{1-6}
        test & 100 & 512 & (550, 650) & 12 & False  \\
    \bottomrule
    \end{tabular}
    \begin{tabular}{cccc}
        \toprule
        & \multicolumn{3}{c}{\textbf{flow speed}} \\
        purpose & bounds / \SI{}{\meter \second^{-1}} & $\tau$ / \SI{}{\second} & vary \\
        \midrule
        train & (0.18, 0.42) & 35 & True \\
        \cline{1-4}
        calibration & (0.18, 0.42) & 35 & True \\
        \cline{1-4}
        test & (0.2, 0.4) & 35 & False \\
    \bottomrule
    \end{tabular}
    \label{case:tab:data-generation-parameters}
\end{table}
\newline
\newline
\noindent
To avoid extrapolation during the test phase, the training and calibration signals are chosen to cover
greater domains of \SI{535}{} to \SI{665}{\kelvin} and \SI{0.18}{} to \SI{0.42}{\meter \second^{-1}}
than the test set. Further, the intrinsic model paramters are sampled for every new simulated trajectory.
The attribute \emph{vary} (tab.~\ref{case:tab:data-generation-parameters}) indicates, whether 
new input or time-varying parameter signals are generated for each simulation trajectory or not. \emph{False}
implies constant signals over all simulation trajectories, that are sampled once at the beginning.
This is important to assess the open-loop simulation capability of the NARX frameworks
for multiple MC-sampled parameter combinations under constant signals. 
